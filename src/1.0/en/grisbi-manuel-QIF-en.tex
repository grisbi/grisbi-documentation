...
For more information on the types of \indexword{account types}\index{types de compte} that Grisbi can manage, see the \vref{accounts-type}, \menu{Grisbi account types} section.

You can define which date will be used for assigning an exercise to
each imported operation, see \vref{setup-general-import-financialyear}, \menu{??Definition of Exercise??}.
...
In the Grisbi \menu{File} menu, choose the option  \menu{Import a
file}, which opens the import wizard window. The import of the account files takes place in five steps :

\begin{enumerate}
	\item Launches the import assistant; confirm with the \menu{Next} button;
	\item selection of the account files to import :	
		\begin{enumerate}
			\item click the button \menu{Add file or files button} : a file manager window opens,	
			\item look for the directory where these account files are,
			\item select one or more account files (with the combination   \key{Ctrl}\key{Click} and \key{Shift}\key{Click}) ; you can also change the \gls{locale} (\gls{character encoding}) of the files to import from the \menu{Coding} drop-down menu,
			\item validate the window to return to the account file selection window,
			\item when the desired files are checked, you can validate with the \menu{Next} button;
		\end{enumerate}		  
	\item complete the import of the files: if everything went well, this window gives the list of the account files which will be imported; continue the import by confirming with the \menu{Next} button;
	\item Review: you can review each account and choose the following actions \ifIllustration \refimage{QIF-import-files-setup-img} :
	% image centrée
	\begin{figure}[htbp]
	\begin{center}
	...
	\fi
		...
		\begin{itemize}
			...
			\item check the scheduled transactions : if \indexword{orphaned transaction}\index{opération !orpheline}  a window will open at the end of the import to know what you want to do: either add them or ignore them,
...
In the \menu{File} menu select the \menu{Export to QIF / CSV / file option\ldots{ }} that opens the Export Accounts Wizard. There are four steps to exporting accounts:

\begin{enumerate}
	\item Starting the assistant; this window indicates that, since the QIF and CSV file formats do not support currency, all transactions will be converted into the currency of their respective account; confirm with the \menu{Next} button;
	\item select the accounts to export by clicking in the corresponding box; confirm with the  \menu{Next} button;
	\item for each account, define the name of the file, the destination directory and the export format, then validate with the \menu{Next} button \ifIllustration \refimage{QIF-export-img} ;
	% image centrée
	\begin{figure}[t]
	\begin{center}
